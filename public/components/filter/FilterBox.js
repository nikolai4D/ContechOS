...
function itemRow(node) {
  ...
  if (
    ...
    childrenFrame = `
            <ul style="margin-bottom:0;" id="acc_${
              node.id
            }-collapse" class="accordion-collapse collapse show" aria-labelledby="acc_${
      node.id
    }-heading">
              <input class="form-check-input" type="checkbox" value="" id="all_${
                node.id
              }"  ${node.viewAll ? "checked" : ""}>
              <label class="form-check-label text-break" for="all_${
                node.id
              }"> All</label>
              <br/>`;
  ...
  if (childrenFrame !== "") {
    mainrow = `
        <div id="acc_${node.id}-heading" class="d-inline-block" role="button">
          ...
          <label class="form-check-label text-break" for="checkbox_${
            node.id
          }"> ${node.title}</label> 
          ${node.selected ? eyeDiv : ""}
          <button type="button" class="btn collapse show" id="acc_${
            node.id
          }-chevron-up" data-bs-toggle="collapse" style="transition-duration: 0s;"
            data-bs-target="#acc_${node.id}-collapse,#acc_${
      node.id
    }-chevron-up,#acc_${node.id}-chevron-down,#acc_${
      node.id
    }-br" aria-expanded="false" aria-controls="acc_${node.id}-collapse">
            ${chevronUp('role="button"')}
          </button>
          <button type="button" class="btn collapse" id="acc_${
            node.id
          }-chevron-down" data-bs-toggle="collapse" style="transition-duration: 0s;"
            data-bs-target="#acc_${node.id}-collapse,#acc_${
      node.id
    }-chevron-up,#acc_${node.id}-chevron-down,#acc_${
      node.id
          ...
          ${childrenFrame}
        <br style="transition-duration: 0s;" id="acc_${
          node.id
        }-br" class="collapse"/>`;
  } else {
    mainrow = `
