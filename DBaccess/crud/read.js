const {getCoordsFromId, getDefTypeFromCoords, getLayerFromId} = require("../idParsers");
const {getItemById, getBulk} = require("../FileManager");
const {Voc} = require("../Voc");

async function readItems(params) {
    ...
    if(sortOfItem !== undefined){
        console.log("itemskinds: " + Voc.itemKinds)
        const sortIndex = Voc.itemKinds.findIndex(el => el === sortOfItem)
        if(sortIndex === -1) {
