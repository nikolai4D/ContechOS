...
const {doesFileExist, getBulk} = require("../FileManager");
...
function getDefTypeFromId(id){
    const layerIndex = getLayerIndexFromAbbr(getAbbrFromId(id))
    const relationTypeFromAbbr = getRelationTypeFromAbbr(getAbbrFromId(id))
    const propertyTypeFromAbbr = getPropertyTypeFromAbbr(getAbbrFromId(id), getKindOfItemFromAbbr(getAbbrFromId(id)))
    return Voc.layers[layerIndex].inString + relationTypeFromAbbr.inString + propertyTypeFromAbbr.inString
}

function getKindOfItemFromId(id){
    return getKindOfItemFromAbbr(getAbbrFromId(id))
}

function getParentIdFromId(id){
    const layerIndex = getLayerIndexFromAbbr(getAbbrFromId(id))
    const propertyType = getPropertyTypeFromAbbr(getAbbrFromId(id), getKindOfItemFromAbbr(getAbbrFromId(id)))
    return getParentId(layerIndex, {parentId: id}, propertyType)
}
...
function getChildrenFromId(id){
    const childrenDefType = getChildrenDefType(getLayerIndexFromAbbr(getAbbrFromId(id)), getRelationTypeFromAbbr(getAbbrFromId(id)), getPropertyTypeFromAbbr(getAbbrFromId(id), getKindOfItemFromAbbr(getAbbrFromId(id))))
    return getBulk([childrenDefType()],-1, 0, filterItems, {parentId: id})
}

function getRelsThisIsTargetOf(id, defType, relationParentId = null, relationTitle = null){
    const filterParams = {target: id}
    if(relationParentId) filterParams.parentId = relationParentId
    if(relationTitle) filterParams.title = relationTitle
    return getBulk([defType + Voc.relationTypes.exRel.inString], - 1, 0, filterItems, filterParams)
}
