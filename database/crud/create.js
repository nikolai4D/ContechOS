...
const {CreateRelationData} = require("../helpers/RelationCreateData");
...
async function createItem(params) {
    try {
        ...
        else if(params.kindOfItem === "relation"){
            const relParams = new CreateRelationData(params.sourceId, params.targetId, params["parentId"])
            id = relParams.id
            defType = relParams.defType
            let fParams = relParams.formattedParams()
            for(let field in fParams){
                formattedParams[field] = fParams[field]
            }
        }
        else if(params.kindOfItem === "property"){
